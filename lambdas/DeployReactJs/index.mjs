...
export const handler = async (event) => {
  const { email, appType, buildTool, repoAccess, accessToken, baseDirectory, environmentVariables } = event;
  ...
  if(!isSubscribed){
    ...
    console.log(`Subscription email send to ${email}: `, subscriptionResponse);
  }

  // fetch docker file from s3
  const dockerFileData = await s3.getObject({
      Bucket: bucketName,
      Key: "dockerfilereactjs"
    }).promise();

  // extract docker file content in string
  let dockerFile = dockerFileData.Body.toString('utf-8').replace("RUN npm run build", envFileContent + "RUN npm run build");
  
  // check the build tool and modify docker file
  if(buildTool === "vite"){
    dockerFile = dockerFile.replace("/app/build", "/app/dist");
  }
